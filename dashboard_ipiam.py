...
try:
    ...
    pass # Permite que o app rode mesmo sem o arquivo .css
...
def carregar_dados(caminho_arquivo):
    ...
    """
    # Adicionado o parâmetro encoding='cp1252' para ler arquivos com acentuação.
    ...
    # Define temas que não são de avaliação para serem excluídos.
    temas_excluidos = ['Classificação por Faixas', 'Classificação Final']
    df_requisitos = df[
        ~df['Tema Principal'].isin(temas_excluidos) &
...
with st.sidebar:
    ...
    # Menu de seleção dos arquivos
    selecao_grau = st.selectbox(
        "Selecione o Grau para Análise:",
        options=list(mapeamento_arquivos.keys())
    )

# --- CARREGAMENTO DINÂMICO DOS DADOS ---
arquivo_selecionado = mapeamento_arquivos[selecao_grau]
try:
    ...
    # --- PÁGINA PRINCIPAL (MAIN HOME) ---

    # TÍTULO DO DASHBOARD
    st.markdown(f"<h1 style='text-align: center;'>📊 Dashboard de Análise: {selecao_grau}</h1>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    with col1:
        ...
        <div style="text-align: center; line-height: 1.2;">
            <div style="font-size: 1rem; color: #84BBD6; margin-bottom: 5px;"><strong>🎯 Pontuação Geral</strong></div>
            <div style="font-size: 2.2rem; font-weight: bold; color: #E0E9F0;">{dados['pontuacao_final']} Pontos</div>
        </div>"""
        st.markdown(pontuacao_html, unsafe_allow_html=True)
    with col2:
        classificacao_html = f"""
        <div style="text-align: center; line-height: 1.2;">
            <div style="font-size: 1rem; color: #84BBD6; margin-bottom: 5px;"><strong>🏆 Classificação do Projeto</strong></div>
            <div style="font-size: 2.2rem; font-weight: bold; color: #E0E9F0;">{dados['classificacao_atual']}</div>
        </div>"""
        st.markdown(classificacao_html, unsafe_allow_html=True)
    with col3:
        requisitos_html = f"""
        <div style="text-align: center; line-height: 1.2;">
            <div style="font-size: 1rem; color: #84BBD6; margin-bottom: 5px;"><strong>✅ Requisitos Atendidos</strong></div>
            <div style="font-size: 2.2rem; font-weight: bold; color: #E0E9F0;">{dados['requisitos_atendidos']} de {dados['total_requisitos']}</div>
        </div>"""
    ...
    with col_graf1:
        ...
        fig_barras = px.bar(dados['df_totais_tema'], x='Presente', y='Tema Principal', orientation='h', text='Presente', color='Tema Principal', color_discrete_sequence=px.colors.qualitative.Pastel)
        fig_barras.update_layout(showlegend=False, yaxis_title=None, xaxis_title="Pontuação", template="plotly_white", autosize=True, margin=dict(l=10, r=10, t=10, b=10), height=350, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color='#84BBD6')
        fig_barras.update_traces(textposition='inside')
        st.plotly_chart(fig_barras, use_container_width=True, config={'displayModeBar': False})

        st.markdown("### **Proporção de Requisitos**")
        df_donut = pd.DataFrame({'Status': ['Presente', 'Ausente'], 'Quantidade': [dados['requisitos_atendidos'], dados['total_requisitos'] - dados['requisitos_atendidos']]})
        fig_donut = px.pie(df_donut, names='Status', values='Quantidade', hole=0.5, color='Status', color_discrete_map={'Presente': 'mediumseagreen', 'Ausente': 'lightcoral'})
        fig_donut.update_traces(textinfo='percent+label', pull=[0.05, 0])
        fig_donut.update_layout(showlegend=False, height=350, margin=dict(t=20, b=20, l=20, r=20), paper_bgcolor='rgba(0,0,0,0)', font_color='#84BBD6')
        st.plotly_chart(fig_donut, use_container_width=True)

    with col_graf2:
        st.markdown("### **Medidor de Classificação Geral**")
        max_score = dados['df_faixas']['Max'].max()
        cores_faixas = ['#2ECC71', '#F1C40F', '#E74C3C', '#9B59B6']
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number", value=dados['pontuacao_final'], domain={'x': [0, 1], 'y': [0, 1]},
            gauge={
                'axis': {'range': [0, max_score], 'tickwidth': 1, 'tickcolor': "darkblue"},
                'bar': {'color': "darkblue"}, 'bgcolor': "rgba(0,0,0,0)", 'borderwidth': 2, 'bordercolor': "gray",
                'steps': [{'range': [row['Min'], row['Max']], 'color': color} for (_, row), color in zip(dados['df_faixas'].iterrows(), cores_faixas)],
                'threshold': {'line': {'color': "red", 'width': 4}, 'thickness': 0.75, 'value': dados['pontuacao_final']}
            }))
        fig_gauge.update_layout(height=350, margin=dict(t=20, b=40, l=40, r=40), paper_bgcolor='rgba(0,0,0,0)', font_color='#84BBD6')
